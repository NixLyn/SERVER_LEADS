...
#    def loop_test(self):
#        for I in range(832348113, 832348120):
#            num_ = "+27"+str(I)
#            print(f"\n*********************\
#                    \n[TEST]:[{num_}]")
#            ret_stack = self.get_stack_(num_)
#            if "ERROR" not in str(ret_stack):
#                time.sleep(2)
#            else:
#                return
...
#MT.loop_test()
    ...
    #def change_ip(self, number_):
    #        g_rl_ = f'https://www.google.com/search?client=firefox-b-e&q={number_}'
    #        std_ = self.scrape_url(g_rl_)
    #        print("[STD_]:",str(std_))
        #with TorRequest() as tr:
        #    response =tr.get('http://ifconfig.me')
        #    print(response.text)
